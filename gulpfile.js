...
function style() {
  return src("src/sass/**/*.+(scss|sass)")
    ...
    ) // Автоматическая расстановка кроссбраузерных префиксов | Automatic placement of cross-browser prefixes
...
function images() {
  return src("src/images/**/*")
    .pipe(
      webp({
        quality: 95, // Качество, близкое к максимальному (100 может дать слишком большой размер)
        lossless: false, // Отключает 100% безпотерьную компрессию (иначе файл может быть больше, чем оригинал)
        alphaQuality: 100, // Максимальное качество альфа-канала (прозрачность)
        method: 6, // Самый продвинутый метод сжатия (максимальное качество)
        sns: 50, // Умеренный баланс между деталями и размером
        filter: 50, // Смягчение артефактов сжатия
        sharpness: 0, // 0 - минимальная резкость, WebP сам регулирует её
      })
    )
...
//video to dev/prod
function video() {
  return src("src/video/**/*").pipe(dest(dir + "/video/"));
}
...
function fonts() {
  return src("src/fonts/**/*").pipe(dest(dir + "/fonts/"));
}
